...
function testCheckpointsResumeAndVerdicts(testCase)
   ...
   figs = dir(fullfile(outdir, 'vdh_*.png'));
   returned = {numel(files), numel(tmpfiles), exist(reportfile, 'file'), ...
      contains(out, 'OVERALL'), contains(text, 'spread(Rd)/se(Rd)'), ...
      contains(text, 'spread(Tt)/se(Tt)'), contains(text, 'sha256'), ...
      contains(text, 'phi_z profile'), sort({figs.name}), ...
      numel(strfind(text, 'figure: '))};
   [~, base] = fileparts(reportfile);
   expected = {36, 0, 2, true, true, true, true, true, ...
      sort({[strrep(base, 'report', 'fluence') '.png'], ...
      [strrep(base, 'report', 'reflect') '.png']}), 2};
